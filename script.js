...
function findCart(name,varn,extra){return cart.find(x=>x.key===name+"|"+varn+"|"+extra)}
function add(name,varn,price,extra=0){
  if(price==null){alert("This item is currently shown as MRP. Please confirm the price with us.");return}
  const key=name+"|"+varn+"|"+extra, f=cart.find(x=>x.key===key);
  if(f)f.qty++; else cart.push({key,name:name+(varn?` (${varn})`:"")+(extra?" + Extra cheese":""),price:price+extra,qty:1});
  renderCart();
}
...
function setItemQty(key,d){change(key,d)}
function renderCats(){
  const nav=document.querySelector("#cats");nav.innerHTML="";
  menu.forEach(([cat])=>{const b=document.createElement("button");b.textContent=cat;b.onclick=()=>document.getElementById("cat-"+slug(cat)).scrollIntoView({behavior:"smooth"});nav.appendChild(b)})
}
  ...
  menu.forEach(([cat,items,note])=>{
    ...
    matches.forEach(x=>{
      ...
      card.querySelector(".add").onclick=()=>{
        const extra=cheese&&cheese.checked?15:0; const variant=sel.value;
        const existing=findCart(name,variant,extra);
        if(existing)existing.qty++; else add(name,variant,variants[variant],extra);
        renderMenu(document.querySelector("#search").value.trim());
        renderCart();
      };
...
function renderCart(){
  ...
  cart.forEach(i=>{total+=i.price*i.qty;count+=i.qty;
    ...
    r.innerHTML=`<div><div class="cart-name">${i.name}</div><div class="cart-sub">₹${i.price} each</div></div><div class="cart-actions"><button type="button">−</button><b>${i.qty}</b><button type="button">+</button></div>`;
...
document.querySelectorAll('input[name="orderType"]').forEach(r=>r.onchange=()=>{
  ...
  const d=r.value==="Delivery",a=document.querySelector("#address");a.required=d;a.disabled=!d;if(!d)a.value="";
});
...
document.querySelector("#orderForm").onsubmit=e=>{
  ...
  window.open(`https://wa.me/919582379369?text=${encodeURIComponent(msg)}`,"_blank");
};
